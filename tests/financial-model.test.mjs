...
import { PLAYERS, contributionTotal, crystalLedger, cycleFund, initialData, recipientBalance, validateData } from "../app/lib/model.ts";
...
test("tracks contributions, delivery debt and available group funds", () => {
  ...
  cycle.recipients[0].contributions.push({ id: "c1", player: "Ardranes", amount: 600_000, at: "2026-08-08", note: "" });
...
test("offsets simultaneous crystal keepers while preserving a zero-sum ledger", () => {
  ...
  assert.deepEqual(before, { Ardranes: -70, Doidinha: 30, xFonseca: 30, Sooul: -20, DeusCriolo: 30 });
...
test("rejects inconsistent recipient order and invalid financial values", () => {
  ...
  data.cycles[0].recipients[0].contributions.push({ id: "bad", player: "Ardranes", amount: -1, at: "", note: "" });
